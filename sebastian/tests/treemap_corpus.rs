//! Corpus test: treemap-beta diagrams vs mermaid-cli (mermaid 11.15.0).
//! All cases must be byte-identical. Cases keep cells reasonably large so the
//! measurement-driven text fitting stays deterministic against Trebuchet.

use sebastian::render_diagram;

#[test]
fn treemap_corpus() {
    let dir = format!("{}/tests/treemap_cases", env!("CARGO_MANIFEST_DIR"));
    let mut cases: Vec<String> = std::fs::read_dir(&dir)
        .expect("treemap_cases dir")
        .filter_map(|e| {
