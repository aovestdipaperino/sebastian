//! Corpus test: quadrantCharts with official mermaid-cli (mermaid 11.15.0)
...
#[test]
fn quadrant_corpus() {
    let dir = format!("{}/tests/quadrant_cases", env!("CARGO_MANIFEST_DIR"));
    let mut cases: Vec<String> = std::fs::read_dir(&dir)
        .expect("quadrant_cases dir")
        .filter_map(|e| {
