//! Corpus test: block-beta diagrams with official mermaid-cli (mermaid
//! 11.15.0) output as the reference. All cases must be byte-identical.
//!
//! Nested composites use the `block:name` form (explicit id) because the
//! anonymous `block` form generates a random id via `Math.random()`.

use sebastian::render_diagram;

#[test]
fn block_corpus() {
    let dir = format!("{}/tests/block_cases", env!("CARGO_MANIFEST_DIR"));
    let mut cases: Vec<String> = std::fs::read_dir(&dir)
        .expect("block_cases dir")
        .filter_map(|e| {
