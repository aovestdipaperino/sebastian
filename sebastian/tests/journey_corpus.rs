//! Corpus test: user-journey diagrams with official mermaid-cli (mermaid 11.15.0)
//! output as the reference. All cases must be byte-identical.

use sebastian::render_diagram;

#[test]
fn journey_corpus() {
    let dir = format!("{}/tests/journey_cases", env!("CARGO_MANIFEST_DIR"));
    let mut cases: Vec<String> = std::fs::read_dir(&dir)
        .expect("journey_cases dir")
        .filter_map(|e| {
